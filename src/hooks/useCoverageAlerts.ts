...
export const useCoverageAlerts = () => {
  return useQuery({
    ...
    queryFn: async (): Promise<CoverageAlert[]> => {
      ...
      // Check for active markets without payment methods
      const { data: allActiveMarkets } = await supabase
        .from('markets')
        .select('id, name')
        .eq('is_active', true);

      const { data: marketsWithPayments } = await supabase
        .from('market_payment_methods')
        .select('market_id')
        .eq('is_active', true);

      const marketsWithPaymentIds = new Set((marketsWithPayments || []).map(m => m.market_id));
      const marketsWithoutPayment = (allActiveMarkets || []).filter(m => !marketsWithPaymentIds.has(m.id));
      ...
      // Check for markets without shipping route
      const marketsWithoutRoute = (allActiveMarkets || []).filter(m => !(m as any).shipping_route_id);
