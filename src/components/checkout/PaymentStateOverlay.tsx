 ...
 * Shows different UI states based on payment status:
 * - Pending: Loading spinner with timer
 * - Pending Validation: MonCash/Transfer instructions
 * - Paid: Confetti and success
 * - Failed/Expired: Error with retry option
 */
...
export const PaymentStateOverlay = ({ onContinue, onCancel }: PaymentStateOverlayProps) => {
    ...
    return (
      <div className="fixed inset-0 bg-background/80 backdrop-blur-sm z-50 flex items-center justify-center">
        <Card className="p-8 text-center max-w-md mx-4">
          ...
          <p className="text-muted-foreground">Verificando estado del pedido...</p>
        </Card>
      </div>
    );
  }
  ...
  const handleRetry = async () => {
    if (activeOrder) {
      await retryPayment(activeOrder.id);
      onContinue?.();
    }
  };
  ...
  // Pending state - Stripe processing
          ...
          {timeRemaining !== null && (
            <div className="mb-6">
              ...
              <p className="text-xs text-muted-foreground mt-2">
                Tu stock está reservado por 30 minutos
              </p>
            </div>
          )}
        ...
        <Card className="p-6 md:p-8 max-w-md w-full my-8">
          ...
          {timeRemaining !== null && (
            <div className="flex items-center justify-center gap-2 mb-4 p-3 bg-yellow-50 rounded-lg">
              ...
              <span className="text-sm text-yellow-800">
                Tu stock está reservado por: <strong>{formatTime(timeRemaining)}</strong>
              </span>
            </div>
          )}
          ...
          {isMonCash ? (
            <div className="space-y-3 mb-6">
              <p className="text-sm font-medium">Envía el pago a:</p>
              <div className="flex items-center justify-between p-3 bg-background border rounded-lg">
                ...
                <Button 
                  size="sm" 
                  variant="ghost"
                  onClick={() => copyToClipboard('+509 3XXX XXXX')}
                >
                ...
                <div className="flex justify-between items-center">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Button 
                      ...
                      onClick={() => copyToClipboard('001-234567-89')}
                    >
          ...
          <div className="bg-blue-50 border border-blue-200 p-4 rounded-lg mb-6">
            <p className="text-sm text-blue-800">
              <strong>¡Tu pedido está reservado!</strong><br />
              El vendedor está validando tu pago. Recibirás una notificación cuando se confirme.
            </p>
          </div>
  ...
  // Paid state - Success with confetti
  if (activeOrder.payment_status === 'paid') {
    return (
      <div className="fixed inset-0 bg-background/95 backdrop-blur-sm z-50 flex items-center justify-center p-4">
        {showConfetti && (
        ...
        <Card className="p-8 text-center max-w-md w-full">
          ...
          <div className="flex items-center justify-center gap-4 mb-6">
            ...
            <div className="text-left">
              ...
              <p className="text-xs text-muted-foreground">Muestra este código al retirar</p>
            </div>
          </div>
        ...
        <Card className="p-8 text-center max-w-md w-full">
          <div className="mb-6">
            ...
            <h2 className="text-xl font-bold mb-2">
              {isExpired ? 'Pedido Expirado' : 'Pago Fallido'}
            </h2>
            <p className="text-muted-foreground">
              {isExpired 
                ? 'El tiempo de reserva ha terminado. El stock ha sido liberado.'
